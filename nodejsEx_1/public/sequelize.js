document.querySelectorAll('#user-list tr').forEach(
    (el) => {
        el.addEventListener('click', function() {
            ...
            getComment(id); // 사용자 지정 함수, 댓글 가져오기
        });
    }
);
...
async function getUser() {
    try {
        const res = await axios.get('/users');// axios: 자바스크립트 외부 통신
...
document.getElementById('user-form').addEventListener('submit',
    async (e) => {
        ...
        //console.log("post name,age, married: ", name, age, married);
        try {
...
async function getComment(id) {
    try {
        const res = await axios.get(`/users/${id}/comments`);// axios: 자바스크립트 외부 통신
